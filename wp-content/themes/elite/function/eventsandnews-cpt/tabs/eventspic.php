...
class Eventspic_tab {
    function get_form($post){
        $post_id = $post->ID;
        $eventspic = get_post_meta($post_id, 'eventspic', true);
        ?>
        ...
        <ul class="sortable" id="icon_reimg" data-rows="<?php echo empty($eventspic) ? 0 : count($eventspic) ?>">
             <?php if(!empty($eventspic)):
                foreach ($eventspic as $key => $id) :
                    $item = get_post($id);
            ?>
            <li class="ui-state-default">
                <input type="hidden" name="eventspic[]" value="<?php echo $id ?>">
...
<!--        <p><button type="button" onclick="update_sticky();" class="button button-primary button-large">Update Settings</button></p>-->
                ...
                $(".btn-reimg").on("click", function(){
                    ...
                    item_image_frame.on('select', function () {
                        ...
                        selection.map(function (attachment) {
                            ...
                            if (attachment.id) {
                                var dup = false;
                                $("#icon_reimg input[name='eventspic[]']").each(function(i,v){
                                ...
                                if(!dup){
                                    ...
                                    '<input type="hidden" name="eventspic[]" value="'+attachment.id+'">'+
    ...
    function save($post_id){
        delete_post_meta($post_id, 'eventspic');
        add_post_meta($post_id, 'eventspic', $_POST['eventspic']);
    }
